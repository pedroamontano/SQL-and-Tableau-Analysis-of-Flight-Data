...
# 2. In total, how many flights were cancelled and how many departed late over both years?
...
Cancelled Flights = 92,363
	...
	These are the total number of flights that were canceled in 2018 and 2019. The data in the Cancelled column shows a "0" for not cancelled, and a "1" for 
	cancelled.
*/

# 3. What were the number of flights cancelled broken down by the cancellation reason?
...
# 4 For each month in 2019, what were the total number of flights and the percentage of flights cancelled?
...
December		275,371						0.51%
	...
	The highest amount of flights per month are during July, August, May, June, and March. The lowest amount of flights are in February. This shows the 
	fluctuation in demand for flights throughout the year. For example, June, July, and August may have more flights due to summer break in schools. Likewise, 
	January and February have the least amount of flights as it is usually when school starts after winter break.
 */
...
# 5. What were the total miles traveled and number of flights broken down by airline in 2018 and 2019 (separately)?
...
# 6. What were the year-over-year percent changes in total flights and miles traveled for each airline?
...
Southwest Airlines Co.		0.84%				-0.12%
	...
	This analysis shows us the percentage increase or decrease in number of flights and miles traveled from 2018 to 2019. The results are specific 
	to each of the three airlines in our data set.
*/

# 7. What investment guidance could br given based on the results?

/* 
Despite the higher percent increase in flights from American Airlines and Delta Airlines, Southwest Airlines has consistently held a higher number of flights in 
2018 and 2019. Additionally, Southwest had a smaller increase in flights from 2018 to 2019 but they reduced their miles flown, thereby reducing flight costs. 
Southwest airlines would be the best investment option due to their consistency in flights flown as well as their strategy in reducing fuel and overhead costs.
*/

# 8. What are the names of the 10 most popular destination airports? 
...
the id for each destination airport. The secondary table with airport information was required to display the airport names. A subquery was used to optimize the
query. In this case, the subquery counts the flight IDs for each destination airport ID and gives us only the 10 destinations with the most flight IDs. After that 
the airports table is joined to get the airport names.
*/

# 9. What were the number of unique aircraft each airline operated in total over 2018-2019?
...
Southwest Airlines Co.		754
	...
	This table shows the total number of planes per airline. This was achieved through the use of the tail numbers on each plane as unique identifiers. Only 
	each distinct tail number is counted to avoid counting the tail number again when it appears in a different flight.
*/

# 10. What is the average distance traveled per aircraft for each of the three airlines?
...
# 11. How do the results on question 9 and 10 impact the estimates of each airline's finances?
...
# 12. Splitting the time of departure by morning, afternoon, evening, and night. What was the average departure delay for each time of day?
...
4.92 min.		4-night
	...
	Evening flights have the highest delay times followed by afternoon flights. This indicates that between noon and 9pm there are a lot more delays than the 
	other time slots. Morning flights are the most common and it is likely that airplanes have flown several times before noon each day. This may cause delays 
	in the afternoon which are carried over to the evening. We see a sharp drop in delay times in the evening when the flight delays have been mostly caught up 
	by the past two time slots.
*/

# 13. What is the average departure delay for each airport and time-of-day combination?
...
# 14. What are the average departure delay for morning delays and airports with at least 10,000 flights?
...
6.6089		1-morning		William P Hobby										34,356
	...
	The results are a little more maneagable than the previous query as this only returns 47 rows. When limited by only morning delays we can see the highest 
	average is 11.07 minutes at San Francisco Internation Airport, which also has 29,517 flights leaving at that time.
*/	

# 15. Which were the top-10 airports with the highest average morning delay?
...
7.77 min.	1-morning		Seattle/Tacoma International			Seattle, WA
	...
	By removing the 10,000 flights limitation the results show the actual highest average delay in the morning per airport. We do see a few of the airports 
	that showed up on the previous query, but the three airports with the highest average delay do not have more than 10,000 flights in the morning.
*/
